...
모델은 더 이상 애플리케이션 코드 안에 박힌 hard-coded string이 아닙니다. 데이터가 됩니다. preset을 바꾸고 compiled registry release을 publish한 뒤 서비스를 reload하면, 다음 요청은 다른 provider나 model을 사용할 수 있습니다. 흔한 model swap에 redeploy가 필요하지 않습니다.
