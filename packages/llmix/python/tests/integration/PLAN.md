...
The active integration gate is limited to OpenAI, OpenRouter, and Sno GPU.
Legacy provider suites remain in the tree for reference, but pytest ignores or
skips Anthropic, Gemini, Novita, DeepInfra, and Together integration calls.

| Provider | Python | TypeScript | Notes |
|----------|--------|------------|-------|
| OpenAI | Yes | Yes | Both have provider implementations |
| OpenRouter | Yes | No | Python provider smoke coverage |
| SnoGPU | Yes | **Python-only** | No TS provider implementation exists |

Tests marked Python-only or with no TypeScript column support have no TS counterpart.

## Prerequisites

- **CI secrets**: OPENAI_API_KEY, OPENROUTER_API_KEY, GPU_BASE_URL, SNO_LLM_API_KEY, OPENAI_KEYS (multi-key)
...
|------|-----------|------|------|
| T1 — Smoke | 1 call per active provider, cache L1, thinking strip | ~$0.05 | Every PR |
...
├── e2e-openai.test.ts
├── test_e2e_openrouter.py         # Suite 5C: OpenRouter      [Py]
├── test_e2e_sno_gpu.py            # Suite 4: SnoGPU           [Py only]
├── test_e2e_anthropic.py          # Ignored legacy suite
├── test_e2e_gemini.py             # Ignored legacy suite
├── test_e2e_novita.py             # Ignored legacy suite
│
...
## Suite 2: Anthropic Real Calls [legacy, ignored]

This suite is retained as historical provider coverage, but it is not part of
the active integration gate.
...
## Suite 3: Gemini Real Calls [legacy, ignored]
...
## Suite 5: Novita Real Calls [legacy, ignored]
...
| 6.7 | L1 miss on different provider — same prompt, OpenAI vs Anthropic [legacy, skipped] | Provider in cache key | dispatch_count==2 |
...
| 6.15 | Cross-provider cache isolation — same prompt to OpenAI and Gemini [legacy, skipped] | Provider in cache key | Different cache entries |
...
| 7.3 | Novita Qwen3.5 stripping — enableThinking=true [legacy, skipped] | Stripping via OpenAI-compat | Same as 7.1 |
| 7.4 | Non-thinking model passthrough — gpt-4o-mini [Py + TS] | No false positives | thinking_content is None, content unchanged |
| 7.5 | Anthropic passthrough — claude-haiku [legacy, skipped] | No false positives on Anthropic | thinking_content is None |
| 7.6 | Gemini passthrough — gemini-2.5-flash [legacy, skipped] | No false positives on Gemini | thinking_content is None |
...
| 10.2 | Anthropic batch — submit 3 prompts, poll, retrieve [legacy, not active gate] | Anthropic batch works | 3 results with content |
| 10.3 | Gemini batch — submit 3 prompts, poll, retrieve [legacy, not active gate] | Gemini batch works | 3 results with content |
...
Active provider integration coverage is OpenAI, OpenRouter, and Sno GPU.
Legacy Anthropic/Gemini/Novita cross-provider calls are not part of the active gate.
...
| Gemini message format | Non-user-last conversation fails | 3.3 (legacy, ignored) |
...
2. **Cheap models** — gpt-4o-mini, OpenRouter smoke aliases, qwen3.6-27b-extract
...
|-------|-------|-----------|
| S1: OpenAI | 12 | $0.01 |
| S2: Anthropic | 0 active / 8 legacy | $0.00 active |
| S3: Gemini | 0 active / 10 legacy | $0.00 active |
| S4: SnoGPU | 12 | $0.00 (on-prem) |
| S5: Novita | 0 active / 6 legacy | $0.00 active |
| S6: Cache | 20 | $0.03 |
| S7: Thinking | 12 | $0.05 |
