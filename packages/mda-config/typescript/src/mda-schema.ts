/**
 * MDA v1.0 source-mode frontmatter JSON Schema (bundled, self-contained).
 *
 * Sourced from `/schemas/frontmatter-source.schema.json` plus the
 * referenced `_defs/*.schema.json` files. Inlined here so the runtime loader
 * has no filesystem dependency on the spec repo.
 *
 * Spec section: §02 (source-permissive shape).
 */

export const MDA_SOURCE_SCHEMA = {
    ...
    dependsOn: {
      ...
      properties: {
        ...
        digest: {
          type: "string",
          pattern:
            "^(sha256:[0-9a-f]{64}|sha384:[0-9a-f]{96}|sha512:[0-9a-f]{128})$",
        },
      },
    ...
    signature: {
      ...
      properties: {
        ...
        "payload-type": {
          type: "string",
          pattern:
            "^application/vnd\\.[a-z0-9][a-z0-9-]*(\\.[a-z0-9][a-z0-9-]*)+\\+json$",
        },
      },
      additionalProperties: false,
      allOf: [
