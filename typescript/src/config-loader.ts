 ...
 *
 * await loader.init();
 * const config = await loader.loadConfig({ module: 'hrkg', preset: 'extraction' });
 * ```
 */
...
const defaultLogger: LLMConfigLoaderLogger = {
  debug: (msg, ...args) => console.debug(`[llmix-config] ${msg}`, ...args),
  info: (msg, ...args) => console.info(`[llmix-config] ${msg}`, ...args),
  warn: (msg, ...args) => console.warn(`[llmix-config] ${msg}`, ...args),
  error: (msg, ...args) => console.error(`[llmix-config] ${msg}`, ...args),
};
...
export class LLMConfigLoader {
   ...
   *
   * @param options - Load options (module, preset, optional scope/userId/version)
  ...
  async loadConfig(options: LoadConfigOptions): Promise<ResolvedLLMConfig> {
    ...
    const experimentResult = await this.getExperimentConfig(module, preset, userId);
    if (experimentResult !== null) {
      version = experimentResult.version;
      forceRefresh = true; // Always bypass cache for experiment configs
      this.logger.info(`[AB] Switched llm:${module}:${preset} to v${version}`);
    }

    // Build primary ConfigId (uses normalized userId and potentially experiment version)
    const configId = this.buildConfigId(scope, module, userId, preset, version);
    ...
    const result = await this.resolveWithCascade(scope, module, userId, preset, version);
  ...
  private buildConfigId(
    ...
    return `${scope}:${module}:${userId}:${preset}:v${version}`;
  }
   ...
   */
  private buildExperimentKey(module: string, preset: string): string {
    return `${EXPERIMENT_KEY_PREFIX}${module}:${preset}`;
  }
   ...
   * @param preset - Preset name (e.g., "extraction")
  ...
  private async getExperimentConfig(
    ...
    try {
      const experimentKey = this.buildExperimentKey(module, preset);
  ...
  private generateCascadeSteps(
    ...
    if (userId !== "_") {
      steps.push({ scope, module, userId, preset, version });
    }

    // Step 2: Module preset (no user)
    steps.push({ scope, module, userId: "_", preset, version });

    // Step 3: Scope-wide fallback (only if module is not _default)
    if (module !== "_default") {
      steps.push({ scope, module: "_default", userId: "_", preset, version });
    }

    // Step 4: Global preset fallback (only if scope is not _default)
    if (scope !== "_default") {
      steps.push({ scope: "_default", module: "_default", userId: "_", preset, version });
    }

    // Step 5: Ultimate base fallback (only if preset is not _base or version is not 1)
    if (preset !== "_base" || version !== 1) {
      steps.push({
        scope: "_default",
        module: "_default",
        userId: "_",
  ...
  private async resolveWithCascade(
    ...
    const steps = this.generateCascadeSteps(scope, module, validUserId, preset, version);
    const originalConfigId = this.buildConfigId(scope, module, validUserId, preset, version);
    ...
    for (const step of steps) {
      try {
        ...
        if (step.preset === "_base" && preset !== "_base") {
          this.logFallbackWarning(originalConfigId, preset, "_base");
        }

        // Check for _low fallback pattern (HRKG use case)
        if (preset.endsWith("_low") && step.preset !== preset) {
          const basePreset = preset.slice(0, -4); // Remove "_low"
          if (step.preset === basePreset) {
            this.logFallbackWarning(originalConfigId, preset, basePreset);
          }
        }
    ...
    throw new ConfigNotFoundError(
      `Config not found after cascade resolution: ${originalConfigId}. ` +
        `Checked: ${steps.map((s) => this.buildConfigId(s.scope, s.module, s.userId, s.preset, s.version)).join(", ")}`
    );
  }
  ...
  private logFallbackWarning(
    ...
    this.logger.warn(
      `LLMConfig fallback: ${requestedPreset} not found, using ${fallbackPreset} (configId: ${configId})`
    );
  }
