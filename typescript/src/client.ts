...
function isHeliconeEnabled(apiKey?: string): boolean {
  const resolvedApiKey = apiKey ?? getHeliconeApiKey();
...
function getHeliconeUrl(provider: "openai" | "anthropic"): string {
  if (provider === "anthropic") {
    return getHeliconeAnthropicBaseUrl();
  }
  return getHeliconeOpenaiBaseUrl();
}

function getHeliconeHeaders(options?: HeliconeHeaderOptions): Record<string, string> {
  const apiKey = options?.apiKey ?? getHeliconeApiKey();
 ...
 * Allows injecting API keys instead of relying on env vars
 */
 ...
 *
 * @param presetString - "module:preset" or "preset"
 * @returns Parsed module and preset
 *
 * @example
 * parsePreset("hrkg:extraction") // { module: "hrkg", preset: "extraction" }
 * parsePreset("extraction") // { module: "_default", preset: "extraction" }
 */
function parsePreset(presetString: string): ParsedPreset {
  const colonIndex = presetString.indexOf(":");
  if (colonIndex === -1) {
    return { module: "_default", preset: presetString };
  }
  return {
    module: presetString.slice(0, colonIndex),
    preset: presetString.slice(colonIndex + 1),
  };
}
 ...
 *
 * Routing logic:
 * - strategy="native" + OpenAI + LLM: Route via Helicone (http://sno-main-1:8585)
...
function getProviderModel(
  ...
  switch (provider) {
    case "openai": {
      const apiKey = apiKeys?.openai ?? getOpenaiApiKey();
      if (!apiKey) {
        throw new Error(
      ...
      if (cachingStrategy === "native" && !isEmbeddingModel(model)) {
        ...
        const heliconeApiKey = helicone?.apiKey ?? getHeliconeApiKey();
    ...
    case "anthropic": {
      const apiKey = apiKeys?.anthropic ?? getAnthropicApiKey();
      if (!apiKey) {
        throw new Error(
    ...
    case "google": {
      const apiKey = apiKeys?.google ?? getGeminiApiKey();
      if (!apiKey) {
        throw new Error(
          "[LLMix] GEMINI_API_KEY environment variable is required for Google provider"
        );
      }
      // Gateway or disabled: use CF AI Gateway or direct
    ...
    case "deepseek": {
      ...
      const apiKey = apiKeys?.openrouter ?? urls?.openRouterApiKey ?? getOpenrouterApiKey();
      if (!apiKey) {
        throw new Error(
      ...
      // LH: Log routing info
      defaultLogger.debug(
        `[LLMix] Routing DeepSeek "${model}" via OpenRouter as "${openRouterModel}"`
      );
...
export class LLMClient {
  ...
  constructor(config: LLMClientConfig) {
    ...
    this.captureTelemetryPayload =
      config.captureTelemetryPayload ?? getCaptureTelemetryPayload() === "true";
    this.callTimeoutMs =
      config.callTimeoutMs ?? (Number(getCallTimeoutMs()) || 120000);
  }

  /**
   * Make an LLM call using resolved config
   *
   * @param options - Call options including preset, messages, and overrides
  ...
  async call(options: CallOptions): Promise<LLMResponse> {
    ...
    // Parse preset string
    const { module, preset } = parsePreset(options.preset);
    ...
    try {
      ...
      this.logger.error(`[LLMix] Config load failed for preset ${options.preset}`, {
        error: errorMessage,
        stack: errorStack,
        module,
        preset,
    ...
    // preset.timeout.totalTime (seconds) → clientConfig.callTimeoutMs (ms) → 120000ms
    const effectiveTimeoutMs = config.timeout?.totalTime
      ? config.timeout.totalTime * 1000 // Convert seconds to ms
      : this.callTimeoutMs;
    ...
    try {
      ...
      this.logger.info(
        `[LLMix] Caching strategy: ${cachingStrategy} for ${options.preset}${effectiveCacheKey ? " (custom key)" : ""}`
      );
      ...
      this.logger.info(
        `[LLMix] Calling ${config.provider}/${effectiveModel} (preset: ${options.preset}, version: ${config.version})`
      );
      ...
      if (usage.inputTokens >= OPENAI_PROMPT_CACHE_MIN_TOKENS) {
        if (usage.cachedInputTokens && usage.cachedInputTokens > 0) {
          ...
          this.logger.info(
            `[LLMix] CACHE HIT | preset=${options.preset} | model=${effectiveModel} | ` +
          ...
          this.logger.info(
            `[LLMix] CACHE MISS | preset=${options.preset} | model=${effectiveModel} | ` +
