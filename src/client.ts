 ...
 */
function resolveCachingStrategy(config: ResolvedLLMConfig, overrideBypassGateway?: boolean): CachingConfig {
	// Priority 1: Legacy override.bypassGateway (backwards compat)
	if (overrideBypassGateway !== undefined) {
		defaultLogger.warn(`[LLMix] DEPRECATED: bypassGateway override used. Use caching.strategy instead.`)
		return overrideBypassGateway ? { strategy: "native", key: config.caching?.key } : { strategy: "gateway" }
	}
 ...
 */
function getProviderModel(provider: Provider, model: string, options?: ProviderRoutingOptions): LanguageModel {
	...
	switch (provider) {
		case "openai": {
			...
			if (!apiKey) {
				throw new Error("[LLMix] OPENAI_API_KEY environment variable is required for OpenAI provider")
			}
			...
			if (cachingStrategy === "native" && !isEmbeddingModel(model)) {
				...
				if (!isHeliconeEnabled(heliconeApiKey)) {
					defaultLogger.warn(`[LLMix] Native caching requested but HELICONE_API_KEY not set. Falling back to gateway.`)
					// Fall through to gateway logic
				} else {
					const heliconeBaseUrl = helicone?.baseUrl ?? getHeliconeUrl("openai")
					defaultLogger.info(`[LLMix] Routing OpenAI via Helicone for native prompt caching`)
		...
		case "anthropic": {
			...
			if (!apiKey) {
				throw new Error("[LLMix] ANTHROPIC_API_KEY environment variable is required for Anthropic provider")
			}
		...
		case "google": {
			...
			if (!apiKey) {
				throw new Error("[LLMix] GOOGLE_GENERATIVE_AI_API_KEY environment variable is required for Google provider")
			}
			// Gateway or disabled: use CF AI Gateway or direct
		...
		case "deepseek": {
			...
			if (urls?.useCfAiGateway && urls.openRouterBaseUrl && cachingStrategy !== "disabled") {
				defaultLogger.debug(`[LLMix] Routing DeepSeek "${model}" via OpenRouter (CF Gateway) as "${openRouterModel}"`)
			} else {
				defaultLogger.debug(`[LLMix] Routing DeepSeek "${model}" via OpenRouter (direct) as "${openRouterModel}"`)
			}
 ...
 */
function deriveCapabilities(config: ResolvedLLMConfig, effectiveModel?: string): ConfigCapabilities {
...
export class LLMClient {
	...
	constructor(config: LLMClientConfig) {
		...
		this.callTimeoutMs = config.callTimeoutMs ?? (Number(process.env.LLMIX_CALL_TIMEOUT_MS) || 120000)
	}
	...
	async call(options: CallOptions): Promise<LLMResponse> {
		...
		try {
			...
			this.logger.error(`[LLMix] LLM call failed for ${config.configId} (${config.provider}/${effectiveModel})`, {
				error: errorMessage,
				stack: errorStack,
	...
	private async trackTelemetryNonBlocking(params: {
		...
		try {
			...
			await Promise.race([this.trackTelemetry(params as Parameters<typeof this.trackTelemetry>[0]), timeoutPromise])
		} catch (error) {
			// Log but never throw - telemetry should not affect response
			this.logger.warn(`[LLMix] Telemetry failed for ${params.config?.configId ?? "unknown"}: ${String(error)}`)
		} finally {
	...
	private async trackTelemetry(params: {
		...
		const { config, effectiveModel, usage, latencyMs, success, errorMessage, messages, output, telemetryContext } =
			params
		...
		const redactedMessages = this.captureTelemetryPayload ? messages : [{ redacted: true, count: messages.length }]
