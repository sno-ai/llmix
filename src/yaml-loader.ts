 ...
 */
export function buildConfigFilePath(configDir: string, module: string, profile: string, version: number): string {
...
export function verifyPathContainment(resolvedPath: string, baseDir: string): void {
	...
	if (rel.startsWith("..") || isAbsolute(rel)) {
		throw new SecurityError(`Path traversal detected: ${resolvedPath} escapes base directory ${baseDir}`)
	}
}
 ...
 * @throws SecurityError if path escapes base directory
 */
export async function verifyPathContainmentAsync(resolvedPath: string, baseDir: string): Promise<void> {
	const normalizedBase = resolve(baseDir)
	const normalizedPath = resolve(resolvedPath)
 ...
 */
export const ProviderSchema = z.enum(["openai", "anthropic", "google", "deepseek"]) satisfies z.ZodType<Provider>
...
export const LLMConfigSchema = z
	...
	.superRefine((data, ctx) => {
		...
		if (data.provider === "anthropic" && data.providerOptions?.anthropic?.thinking?.type === "enabled") {
		...
		if (data.bypassGateway !== undefined && data.caching !== undefined) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				message: "Both bypassGateway (deprecated) and caching are set. Use caching.strategy instead of bypassGateway.",
				path: ["bypassGateway"],
			})
		}
