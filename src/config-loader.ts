...
export class LLMConfigLoader {
	private readonly config: Required<
		Pick<LLMConfigLoaderConfig, "configDir" | "cacheSize" | "cacheTtlSeconds" | "redisTtlSeconds" | "defaultScope">
	> &
	 ...
	 */
	private buildConfigId(scope: string, module: string, userId: string, profile: string, version: number): string {
	...
	private async resolveWithCascade(
		...
		for (const step of steps) {
			try {
				...
				const config = await loadConfigFromFile(this.config.configDir, step.module, step.profile, step.version)

				const resolvedConfigId = this.buildConfigId(step.scope, step.module, step.userId, step.profile, step.version)
	 ...
	 */
	private logFallbackWarning(configId: string, requestedProfile: string, fallbackProfile: string): void {
	...
	private async tryLoadFromRedis(configId: string): Promise<ResolvedLLMConfig | null> {
		...
		try {
			...
			const errorMessage = error instanceof Error ? error.message : String(error)
			this.logger.warn(`Redis fetch failed for ${configId}: ${errorMessage}. Falling back to file.`)
