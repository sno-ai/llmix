...
try:
    ...
    class ProviderCircuitBreaker:
        ...
        @classmethod
        def for_provider(
            cls, provider: str, config: ProviderCircuitBreakerConfig | None = None
            ...
            with cls._instances_lock:
                ...
                if existing is None:
                    ...
                    logger.warning(
                        "[CB:%s] ignoring mismatched config for existing singleton (requested=%s, existing=%s)",
                        key,
                        config,
                        existing._config,
                    )
            ...
            with self._lock:
                ...
                if self._state == CircuitState.OPEN:
                    ...
                    if elapsed >= self._config.recovery_timeout:
                        ...
                        logger.info(
                            "[CB:%s] OPEN -> HALF_OPEN (%.0fs elapsed, probing primary)",
                            self._provider,
                            elapsed,
                        )
                        return True
